...
#looking for essential genes MAGeck file
mageck_file <- "data/raw/41467_2018_5506_MOESM6_ESM.xlsx"

bjab_mageck<- read_excel(mageck_file,
                         sheet="GECKO_BJAB")
dim(bjab_mageck)
names(bjab_mageck)
#test
bjab_mageck[bjab_mageck$id=="CTDP1",
            c("id", "num", "neg|fdr", "neg|lfc", "neg|goodsgrna")]

bjab_fdr_sig<- bjab_mageck[bjab_mageck$`neg|fdr`<0.05,]
dim(bjab_fdr_sig)


# modeling

glimpse(gecko_counts)
gecko_counts$mean_day0<-mean(gecko_counts$c(`BC-3_Day0_Rep1`,`BC-3_Day0_Rep2`, `BC-3_Day0_Rep3`)
