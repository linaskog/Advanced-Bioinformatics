...
# Merging Data
gecko <- merge(gecko, sequences, c("UID"))
...
genes <- unique(gecko_model$Gene.y)

train_genes <- sample(genes, floor(0.8 * length(genes)))
test_genes <- setdiff(genes, train_genes)

train_index <- which(gecko_model$Gene.y %in% train_genes)
test_index <- which(gecko_model$Gene.y %in% test_genes)
